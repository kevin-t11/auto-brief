...
export default function SignIn() {
    ...
    await authClient.signIn.email(
      ...
      },
      {
        ...
        onSuccess: async () => {
          ...
          router.refresh();
          toast.success('Signed in successfully! Hang tight...');
        },
        onError: (ctx: ErrorContext) => {
          ...
          if (
            ...
            toast.error('Please verify your email before signing in.', {
              action: {
                label: resendingEmail ? 'Sending...' : 'Resend Email',
            ...
            ctx.error.message?.toLowerCase().includes('no user found') ||
            ctx.error.message?.toLowerCase().includes('user not found') ||
            ctx.error.code === 'USER_NOT_FOUND'
          ) {
            toast.error("User not found with this email address. Don't have an account? Sign up.", {
              action: {
                label: 'Sign Up',
  ...
  return (
    <div className="flex items-center justify-center min-h-screen md:pt-10 mx-5 md:mx-0">
      <Card className="w-full max-w-md mx-auto bg-neutral-50 rounded-md shadow-md">
        <CardHeader>
